...
function ApplicationBuilder(callback) {
	...
	Application.bind("Promise", function (cb, module) {
		...
		var resolve		= function (v) {
			if (pending) {
				// Promises/A+ 2.3 - thenable adoption: if the fulfillment
				// value is itself a thenable (e.g. a Promise returned from a
				// `.then` handler), adopt its eventual state instead of
				// resolving with the thenable object directly.
				if (v === p) {
					reject(new TypeError("Chaining cycle detected: a Promise cannot be resolved with itself"));
					return;
				}
